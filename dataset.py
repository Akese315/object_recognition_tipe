...
class CardRecognitionDataset(Dataset):
    def __init__(
        self,
        directory,
        is_dark_and_white=False,
        reduction_factor=8,
        bounding_boxes_ratio=[],
        max_width=800,
        max_height=800,
            ...
            [
                ...
                Albu.OneOf(
                    [
                        Albu.OpticalDistortion(
                            distort_limit=0.05, shift_limit=0.05, p=1
                        ),
                ...
                Albu.OneOf(
                    [
                        ...
                        Albu.ISONoise(
                            color_shift=(0.01, 0.05), intensity=(0.1, 0.5), p=1
                        ),  # CORRECTION ICI (pas d'espace)
                        Albu.ToGray(p=1),
                    ],
                    p=0.2,
                ),
                ...
                Albu.OneOf(
                    [
                        ...
                        Albu.RandomFog(
                            fog_coef_lower=0.3, fog_coef_upper=0.5, alpha_coef=0.1, p=1
                        ),
                    ],
                    p=0.1,
                ),
    ...
    def transform_image(self, image: CustomImage) -> torch.Tensor:
        ...
        label = torch.zeros(
            grid_division_y, grid_division_x, len(self.bboxes_ratio), 5 + self.n_classes
        )
        ...
        for box in bb_boxes:
            ...
            xc, yc, w, h = (
                coordinates.x_center,
                coordinates.y_center,
                coordinates.width,
                coordinates.height,
            )
        ...
        augmented = self.transform(
            image=image_np, bboxes=bb_boxes_yolo_format, class_labels=bb_boxes_classes
        )

        aug_image = image_tensor = transforms.ToTensor()(
            augmented["image"]
        ...
        for i in range(len(aug_boxes)):
            ...
            box = BoundingBox(
                True, x_center, y_center, width, height, box.class_id, box.num_classes
            )
            ...
            label[new_coordinates[0], new_coordinates[1], anchor_index] = (
                box.get_tensor()
            )
            boxes.append(box)

            new_image = CustomImage.from_tensor(
                image_tensor=aug_image,
                file_name=image.file_name,
                bounding_boxes=boxes,
                target_size=image.resized_size,
                reduction_factor=self.reduction_factor,
            )
...
class CustomBatchSampler(Sampler):
    def __init__(
        self,
        dataset: CardRecognitionDataset,
        batch_size: int,
