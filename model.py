...
class YOLOHead(nn.Module):
    ...
    def forward(self, x):
        ...
        # Coordonnées finales
        bx = torch.sigmoid(tx) # Normalisation en [0,1]
        by = torch.sigmoid(ty) # Normalisation en [0,1]
        bw = torch.sigmoid(tw)  # Borne en (0,1)
        bh = torch.sigmoid(th)  # Borne en (0,1)
...
class YoloLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        # suppose que preds, targets = [B, S, S, A, 5+C]
        # [obj, x, y, w, h, class0, class1]
        obj_mask = targets[..., 0] == 1   # [B,S,S,A]
        noobj_mask = targets[..., 0] == 0

        # Coordonnées uniquement sur les cellules avec objet
        ...
        total = (
            ...
            loss_class
        ) / preds.size(0)  # moyenne par batch
